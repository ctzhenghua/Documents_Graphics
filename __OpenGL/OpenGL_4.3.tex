	...
	\section{帧缓冲}
		...
		\subsection{使用}
			\dirtree{%
				...
				.2 glBindFramebuffer(GL\_FRAMVYFFER, fbo) // 使用这个帧缓冲 .
				.2 绑定一个附件(将这帧渲染至帧缓冲中的何处) .
				.3 glFramebufferTexture2D(GL\_FRAMEBUFFER, GL\_COLOR\_ATTACHMENT0, GL\_TEXTURE\_2D, texture, 0) // 用户可读模式 .
				.3 glFramebufferRenderbuffer(GL\_FRAMEBUFFER, GL\_DEPTH\_STENCIL\_ATTACHMENT,\\ GL\_RENDERBUFFER, rbo) // 原生，速度快，只写 .
				.2 glBindFramebuffer(GL\_FRAMEBUFFER, 0) // 使用默认帧缓冲 .
				.2 glDeleteFramebuffers(1, \&fbo) .
			}
